...
begin
    ...
    select mail into MailSignaleur from signalement_signaleur where fk_id_signalement = new.id_resource;
    if MailSignaleur <> ''
       then
         UPDATE signalement_export set mail_usager = MailSignaleur where id_signalement = new.id_resource;
    end if;
