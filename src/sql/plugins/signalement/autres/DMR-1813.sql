...
begin
		 ...
		 where sors.fk_id_signalement=new.id_signalement) || (select observation_rejet_comment from signalement_observation_rejet_signalement where fk_id_signalement=new.id_signalement and observation_rejet_comment is not null),', ') into NewRaisonRejet ;
...
update
	signalement_export
set
	raisons_rejet = observation_rejet_comment
from
	(
		select
			id_signalement,
			observation_rejet_comment
		from
			signalement_export export join signalement_observation_rejet_signalement rejet_signalement on
			rejet_signalement.fk_id_signalement = export.id_signalement
		where
			id_wkf_state = 11
			and raisons_rejet = ''
			and rejet_signalement.fk_id_observation_rejet is null
	) as subquery
where
	signalement_export.id_signalement = subquery.id_signalement;